            ...
            <div class="flex justify-between items-center h-20">
                ...
                <div class="hidden md:flex space-x-8 items-center">
                    <template v-for="link in links">
                        <Link :href="route(link.target)" class="text-sm font-medium hover:text-brand-600 transition" :class="route().current(link.active) ? 'text-brand-600' : 'text-slate-600'">{{ link.text }}</Link>
                    </template>
                </div>

                <div class="hidden md:flex items-center gap-4">
                    <Link href="#" class="text-sm font-medium text-slate-600 hover:text-brand-600">Masuk</Link>
                    <Link href="#"
