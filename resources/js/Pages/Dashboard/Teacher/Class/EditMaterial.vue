...
<template>
    ...
    <div class="space-y-6 pb-20">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
            <Link :href="route('teacher.dashboard.class.index')"
        ...
        <div class="border-b border-slate-200">
            <nav class="flex gap-8">
                ...
                <button @click="activeTab = 'murid'" class="pb-4 text-sm font-bold border-b-2 transition"
                    :class="activeTab === 'murid' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-700'">
                    Murid
                </button>
            </nav>
        </div>
        ...
        <div v-if="activeTab === 'curriculum'" class="max-w-4xl mx-auto space-y-8">
            ...
            <div class="flex justify-between items-center">
                ...
                <Link :href="route('teacher.dashboard.class.create.session')"
            ...
            <div class="space-y-6">
                ...
                <div v-for="(session, sIndex) in sessions" :key="session.id"
                    ...
                    <div class="bg-slate-100/50 p-4 flex items-center justify-between border-b border-slate-200 group">
                        ...
                        <div class="flex items-center gap-2">
                            <Link :href="route('teacher.dashboard.class.edit.session')" class="p-1.5 text-slate-400 hover:text-indigo-600 transition">
                                <PencilSquareIcon class="w-4 h-4" />
                            </Link>
                            <Link href="#" class="p-1.5 text-slate-400 hover:text-red-500 transition">
                ...
                <Link :href="route('teacher.dashboard.class.create.session')"
                    class="cursor-pointer w-full py-4 border-2 border-dashed border-slate-300 rounded-2xl text-slate-400 font-bold hover:border-indigo-500 hover:text-indigo-600 hover:bg-indigo-50 transition flex flex-col items-center justify-center gap-2">
