...
import {
    ...
    TrashIcon, PlusIcon, KeyIcon, ChevronDownIcon,
    ChevronUpIcon
...
// Toggle bank expand
const toggleBankExpand = (id) => {
    if (expandedBankIds.value.includes(id)) {
        expandedBankIds.value = expandedBankIds.value.filter(itemId => itemId !== id);
    } else {
        expandedBankIds.value.push(id);
    }
};
...
const submit = () => {
    ...
    // form.post(...)
    alert("Assignment beserta soal berhasil disimpan (Cek Console)!");
};
        ...
        <form @submit.prevent="submit" class="space-y-8">
            ...
            <div class="bg-white p-6 md:p-8 rounded-2xl border border-slate-200 shadow-sm">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-2">Instruksi Assignment</label>
                        <div class="prose-editor-wrapper">
                            <QuillEditor v-model:content="form.content" contentType="html" theme="snow" toolbar="full"
                                placeholder="Tulis instruksi assignment di sini..." />
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div v-if="showBankModal" class="fixed inset-0 z-[999] flex items-center justify-center p-4">
            ...
            <div
                ...
                <div class="p-5 flex-1 overflow-y-auto bg-slate-50 custom-scrollbar">
                    ...
                    <div class="space-y-2">
                        <div v-for="q in filteredBankQuestions" :key="q.id" @click="toggleSelectBank(q.id)" :class="{
                            'border-indigo-500 bg-indigo-50 ring-1 ring-indigo-500': selectedBankIds.includes(q.id),
                            'border-slate-200 bg-white hover:border-indigo-300': !selectedBankIds.includes(q.id)
                        }" class="p-4 rounded-xl border cursor-pointer transition flex gap-3 items-start select-none relative group">

                            <div :class="selectedBankIds.includes(q.id) ? 'bg-indigo-600 border-indigo-600' : 'bg-white border-slate-300'"
                                class="w-5 h-5 rounded border flex items-center justify-center mt-0.5 transition-colors flex-shrink-0">
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex gap-2 mb-1.5 flex-wrap">
                                    <span
                                        :class="q.type === 'choice' ? 'text-blue-600 bg-blue-100' : 'text-orange-600 bg-orange-100'"
                                        class="text-[10px] uppercase font-bold px-2 py-0.5 rounded">
                                        {{ q.type === 'choice' ? 'Pilihan Ganda' : 'Essay' }}
                                    </span>
                                ...
                                <p class="text-sm text-slate-800 font-medium mb-2">{{ q.text }}</p>

                                <div v-if="q.type === 'choice'">
                                    <button type="button" @click.stop="toggleBankExpand(q.id)"
                                        class="text-xs font-bold flex items-center gap-1 text-indigo-600 hover:text-indigo-800 hover:bg-indigo-100 px-2 py-1 rounded-lg transition -ml-2">
                                        <component
                                            :is="expandedBankIds.includes(q.id) ? ChevronUpIcon : ChevronDownIcon"
                                            class="w-3 h-3" />
                                        {{ expandedBankIds.includes(q.id) ? 'Sembunyikan Opsi' : 'Lihat Opsi Jawaban' }}
                                    </button>

                                    <div v-if="expandedBankIds.includes(q.id)"
                                        class="mt-2 pl-2 border-l-2 border-slate-200 space-y-1 animate-fade-in">
                                        <div v-for="(opt, idx) in q.options" :key="idx"
                                            class="flex items-center gap-2 text-xs">
                                            <div :class="q.correct_option === idx ? 'bg-emerald-500 border-emerald-500' : 'bg-white border-slate-300'"
                                                class="w-2 h-2 rounded-full border flex-shrink-0"></div>

                                            <span
                                                :class="{ 'font-bold text-emerald-700': q.correct_option === idx, 'text-slate-600': q.correct_option !== idx }">
                                                {{ opt }}
                                            </span>

                                            <span v-if="q.correct_option === idx"
                                                class="text-[10px] text-emerald-600 font-bold bg-emerald-50 px-1.5 rounded ml-1">
                                                Kunci
                                            </span>
                                        </div>
                                    </div>
